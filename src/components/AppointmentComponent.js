...
//   // ✅ NEW: fetch tutors for dropdown (public)
//   useEffect(() => {
...
//   // ✅ tutor full name (THIS IS WHAT WE SEND TO BACKEND)
...
//       duration: Number(av.slotDuration ?? 2), // hours
//     };
//   }, [selectedTutor]);

//   // ✅ ONLY AUTO-FILL IF SESSION VALID (NO ALERT / NO REDIRECT)
//   useEffect(() => {
//     if (!isSessionValid()) {
//       // Not logged in: do nothing, just keep fields empty
//       setName("");
//       setEmail("");
//       setUserBookedSlots([]);
//       return;
...
//     // fetch /me fresh (optional but good)
//     const token = localStorage.getItem("token");
...
//     // fallback if local email exists
//     if (localEmail) {
...
//   // ✅ reset slot selection when tutor/date changes
//   useEffect(() => {
//     setStartTime("");
//     setEndTime("");
//     // ✅ also reset subject when tutor changes (so subject dropdown matches tutor)
//     setSubject("");
//   }, [tutor, date]);

//   // ✅ fetch tutor/date booked slots (USE tutorName)
//   useEffect(() => {
//     if (tutorName && date && !isSunday(date)) {
//       fetch(
...
//     if (!date || !tutor || isSunday(date)) return [];
...
//   // ✅ IMPORTANT: alert+redirect ONLY ON SUBMIT
...
//         "Booking કરવા માટે login/register જરૂરી છે. 3 second માં Login page પર લઈ જઈએ છીએ..."
//       );
//       return;
//     }
...
//       // ✅ refresh tutor/date bookings (USE tutorName)
...
//                 <b>Note:</b> Booking કરવા માટે login/register જરૂરી છે.
//               </div>
//             )}
//           </div>
...
//                             setEndTime(slot.end);
//                           }}
//                           className={`btn ${slot.isBooked ? "btn-danger" : "btn-outline-primary"}`}
//                         >
//                           {slot.start} - {slot.end}
//                         </button>
//                       ))}
...
//                     {tutor && date && !isSunday(date) && generateTimeSlots().length === 0 ? (
